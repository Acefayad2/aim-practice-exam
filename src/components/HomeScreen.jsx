  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f0fa', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 16px' }}>
          ...
          >
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: PURPLE, letterSpacing: 2, textTransform: 'uppercase' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {[
                ...
                <div key={label} style={{ background: LIGHT_PURPLE, borderRadius: 8, padding: '10px 12px' }}>
                  <div style={{ fontSize: 11, color: '#4a5568', fontWeight: 600, marginBottom: 2 }}>{label}</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: PURPLE }}>{value}</div>
                </div>
              ))}
            </div>
      ...
      <p style={{ marginTop: 40, color: '#a0aec0', fontSize: 13, textAlign: 'center' }}>
        AIM · Maryland Insurance Producer Licensing Prep
      </p>
    </div>
  )
}
